...
def test_routejson_validation_errors():
    ...
    assert client.get("/routejson?startaddress=A&endaddress=B").status_code == 400
...
def test_routejson_requires_ors_key(monkeypatch):
    ...
    response = server.app.test_client().get("/routejson?startaddress=A&endaddress=B&routemode=driving-car")
